...
import static jp.sourceforge.andjong.mahjong.EventIf.EventId;
import static jp.sourceforge.andjong.mahjong.EventIf.KAZE_NAN;
import static jp.sourceforge.andjong.mahjong.EventIf.KAZE_NONE;
import static jp.sourceforge.andjong.mahjong.EventIf.KAZE_TON;
...
public class Mahjong implements Runnable {
    ...
    public int getRelation(int fromKaze, int toKaze) {
        int relation;
        if (fromKaze == toKaze) {
    ...
    private void startKyoku() {
        ...
        while (true) {
            ...
            if (m_tsumoHai == null) {
                ...
                for (int i = 0, j = m_iOya; i < m_players.length; i++, j++) {
                    ...
                    if (agari) {
                        ...
                        m_score.setNagashiMangan(m_agariInfo, m_view
                                .getResources());

                        iPlayer = m_kazeToPlayerIdx[m_kazeFrom];
                        if (m_iOya == iPlayer) {
                            score = m_agariInfo.m_score.m_oyaRon + (m_honba
                                    * 300);
                            for (int l = 0; l < 3; l++) {
                                iPlayer = (iPlayer + 1) % 4;
                                m_players[iPlayer].reduceTenbou(
                                        m_agariInfo.m_score.m_oyaTsumo
                                                + (m_honba * 100));
                            }
                        } else {
                            score = m_agariInfo.m_score.m_koRon + (m_honba
                                    * 300);
                            for (int l = 0; l < 3; l++) {
                                iPlayer = (iPlayer + 1) % 4;
                                if (m_iOya == iPlayer) {
                ...
                for (int i = 0; i < m_tenpai.length; i++) {
                    if (m_tenpai[i]) {
                        m_players[m_kazeToPlayerIdx[i]].increaseTenbou(
                                incScore);
                    } else {
            ...
            case TSUMO_AGARI:// ツモあがり
                ...
                m_score = new AgariScore();
                m_score.getAgariScore(activePlayer.getTehai(), m_tsumoHai,
                        combis, m_setting, m_agariInfo, m_view.getResources());
                ...
                if (m_iOya == iPlayer) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        iPlayer = (iPlayer + 1) % 4;
                        m_players[iPlayer].reduceTenbou(
                                m_agariInfo.m_score.m_oyaTsumo + (m_honba
                                        * 100));
                    }
                } else {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (m_iOya == iPlayer) {
                            ...
                            m_players[iPlayer].reduceTenbou(
                                    m_agariInfo.m_score.m_koTsumo + (m_honba
                                            * 100));
                        }
                    }
                }
            ...
            case RON_AGARI:// ロン
                ...
                m_score = new AgariScore();
                m_score.getAgariScore(activePlayer.getTehai(), m_suteHai,
                        combis, m_setting, m_agariInfo, m_view.getResources());
     ...
     */
    private void haipai() {
        for (int i = 0, j = m_iOya, max = m_players.length
                * 13; i < max; i++, j++) {
            if (j >= m_players.length) {
                j = 0;
            }
    ...
    private EventId tsumoEvent() {
        ...
        // イベント（ツモ）を発行する。
        EventId retEid = activePlayer.getEventIf().event(EventId.TSUMO,
                m_kazeFrom, m_kazeFrom);
        Log.i(TAG, retEid.toString() + ", kazeFrom = " + m_kazeFrom
                + ", kazeTo = " + m_kazeTo);
        ...
        case ANKAN:
            ...
            activePlayer.getTehai().setAnKan(kanHais[sutehaiIdx], getRelation(
                    this.m_kazeFrom, this.m_kazeTo));

            // イベントを通知する。
            notifyEvent(EventId.ANKAN, m_kazeFrom, m_kazeFrom);
        ...
        case SUTEHAI:// 捨牌
            ...
            if (sutehaiIdx >= activePlayer.getTehai().getJyunTehaiLength()) {// ツモ切り
                ...
                activePlayer.getTehai().copyJyunTehaiIndex(m_suteHai,
                        sutehaiIdx);
     ...
     * @param a_kazeFrom イベントを発行した風
     * @param a_kazeTo イベントの対象となった風
     * @return イベントID
     */
    private EventId notifyEvent(EventId a_eventId, int a_kazeFrom,
            int a_kazeTo) {
        // UIイベントを発行する。
        ...
        case SUTEHAI:
        case REACH:
            for (int i = 0, j = a_kazeFrom + 1; i < m_players.length
                    - 1; i++, j++) {
                if (j >= m_players.length) {
                    j = 0;
                }
                ...
                ret = activePlayer.getEventIf().event(EventId.RON_CHECK,
                        a_kazeFrom, j);
        ...
        NOTIFYLOOP: for (int i = 0, j = a_kazeFrom; i < m_players.length; i++, j++) {
            ...
            // イベントを発行する。
            a_kazeTo = j;
            ret = activePlayer.getEventIf().event(a_eventId, a_kazeFrom,
                    a_kazeTo);
            ...
            case PON:
                ...
                this.m_kazeTo = a_kazeFrom;
                activePlayer = m_players[m_kazeToPlayerIdx[this.m_kazeFrom]];
                activePlayer.getTehai().setPon(m_suteHai, getRelation(
                        this.m_kazeFrom, this.m_kazeTo));
                m_players[m_kazeToPlayerIdx[this.m_kazeTo]].getKawa().setNaki(
                        true);

                notifyEvent(EventId.SELECT_SUTEHAI, this.m_kazeFrom,
                        this.m_kazeTo);
            ...
            case CHII_LEFT:
                ...
                this.m_kazeTo = a_kazeFrom;
                activePlayer = m_players[m_kazeToPlayerIdx[this.m_kazeFrom]];
                activePlayer.getTehai().setChiiLeft(m_suteHai, getRelation(
                        this.m_kazeFrom, this.m_kazeTo));
                m_players[m_kazeToPlayerIdx[this.m_kazeTo]].getKawa().setNaki(
                        true);
                ...
                // イベントを通知する。
                ret = notifyEvent(EventId.CHII_LEFT, this.m_kazeFrom,
                        this.m_kazeTo);
                break NOTIFYLOOP;
            case CHII_CENTER:
                m_isChiihou = false;
                ...
                this.m_kazeTo = a_kazeFrom;
                activePlayer = m_players[m_kazeToPlayerIdx[this.m_kazeFrom]];
                activePlayer.getTehai().setChiiCenter(m_suteHai, getRelation(
                        this.m_kazeFrom, this.m_kazeTo));
                m_players[m_kazeToPlayerIdx[this.m_kazeTo]].getKawa().setNaki(
                        true);
                ...
                // イベントを通知する。
                ret = notifyEvent(EventId.CHII_CENTER, this.m_kazeFrom,
                        this.m_kazeTo);
                break NOTIFYLOOP;
            case CHII_RIGHT:
                m_isChiihou = false;
                ...
                this.m_kazeTo = a_kazeFrom;
                activePlayer = m_players[m_kazeToPlayerIdx[this.m_kazeFrom]];
                activePlayer.getTehai().setChiiRight(m_suteHai, getRelation(
                        this.m_kazeFrom, this.m_kazeTo));
                m_players[m_kazeToPlayerIdx[this.m_kazeTo]].getKawa().setNaki(
                        true);
                ...
                // イベントを通知する。
                ret = notifyEvent(EventId.CHII_RIGHT, this.m_kazeFrom,
                        this.m_kazeTo);
                break NOTIFYLOOP;
            case DAIMINKAN:
                m_isChiihou = false;
                ...
                this.m_kazeTo = a_kazeFrom;
                activePlayer = m_players[m_kazeToPlayerIdx[this.m_kazeFrom]];
                activePlayer.getTehai().setDaiMinKan(m_suteHai, getRelation(
                        this.m_kazeFrom, this.m_kazeTo));
                m_players[m_kazeToPlayerIdx[this.m_kazeTo]].getKawa().setNaki(
                        true);

                // イベントを通知する。
                notifyEvent(EventId.DAIMINKAN, this.m_kazeFrom, this.m_kazeTo);
    ...
    void copyTehai(Tehai tehai, int kaze) {
        if (activePlayer.getJikaze() == kaze) {
            ...
            Tehai.copy(tehai, m_players[m_kazeToPlayerIdx[kaze]].getTehai(),
                    false);
        }
    }
    ...
    public int getAgariScore(Tehai tehai, Hai addHai) {
        ...
        m_score = new AgariScore();
        int score = m_score.getAgariScore(tehai, addHai, combis, setting,
                m_agariInfo, m_view.getResources());
        return score;
    }
