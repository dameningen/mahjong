    ...
    public EventId event(EventId eid, int a_kazeFrom, int a_kazeTo) {
        ...
        case TSUMO:
            ...
            while (true) {
                ...
                m_info.postUiEvent(EventId.UI_INPUT_PLAYER_ACTION, a_kazeFrom,
                        a_kazeTo);
                ...
                if (m_playerAction.isDispMenu()) {
                    ...
                    if ((menuSelect >= 0) && (menuSelect < menuNum)) {
                        ...
                        if (eventId[menuSelect] == EventId.REACH) {
                            ...
                            while (true) {
                                // 入力を待つ。
                                m_playerAction.setState(
                                        PlayerAction.STATE_REACH_SELECT);
                                m_info.postUiEvent(
                                        EventId.UI_INPUT_PLAYER_ACTION,
                                        a_kazeFrom, a_kazeTo);
                                m_playerAction.actionWait();
                            ...
                            if (kanNum > 1) {
                                while (true) {
                                    m_playerAction.init();
                                    // 入力を待つ。
                                    m_playerAction.setValidKan(false, kanHais,
                                            kanNum);
                                    // m_playerAction.setChiiEventId(eventId[iChii]);
                                    m_playerAction.setState(
                                            PlayerAction.STATE_KAN_SELECT);
                                    m_info.postUiEvent(
                                            EventId.UI_INPUT_PLAYER_ACTION,
                                            a_kazeFrom, a_kazeTo);
                                    m_playerAction.actionWait();
                                    int kanSelect = m_playerAction
                                            .getKanSelect();
                                    m_playerAction.init();
        ...
        case RON_CHECK:
            ...
            if (!furiten) {
                ...
                if (agariScore > 0) {
                    ...
                    m_info.postUiEvent(EventId.UI_INPUT_PLAYER_ACTION,
                            a_kazeFrom, a_kazeTo);
                    m_playerAction.actionWait();
                    int menuSelect = m_playerAction.getMenuSelect();
                    if (menuSelect < 1) {
            ...
            if (!m_info.isReach() && (m_info.getTsumoRemain() > 0)) {
                ...
                if ((relation == -1) || (relation == 3)) {
                    ...
                    if (m_tehai.validChiiCenter(suteHai, sarashiHaiCenter)) {
                        m_playerAction.setValidChiiCenter(true,
                                sarashiHaiCenter);
                        if (chiiCount == 0) {
                            iChii = menuNum;
                    ...
                    if ((eventId[menuSelect] == EventId.CHII_LEFT)
                        ...
                        if (chiiCount > 1) {
                            while (true) {
                                ...
                                m_playerAction.setChiiEventId(eventId[iChii]);
                                m_playerAction.setState(
                                        PlayerAction.STATE_CHII_SELECT);
                                m_info.postUiEvent(
                                        EventId.UI_INPUT_PLAYER_ACTION,
                                        a_kazeFrom, a_kazeTo);
                                m_playerAction.actionWait();
                                EventId chiiEventId = m_playerAction
                                        .getChiiEventId();
                                m_playerAction.init();
                                return chiiEventId;
                            }
