...
public class AI implements EventIf {
    ...
    public EventId event(EventId a_eventId, int a_kazeFrom, int a_kazeTo) {
        ...
        case RON_CHECK:
        case REACH:
            eventId = eventSutehai(a_kazeFrom);
            break;
        case SELECT_SUTEHAI:
     ...
     * @return イベントID
     */
    private EventId eventSutehai(int a_kazeFrom) {
    ...
    private boolean isFuriten() {
        ...
        if (indexNum > 0) {
            ...
            if (!furiten) {
                ...
                FURITENLOOP: for (; playerSuteHaisCount < suteHaisCount
                        - 1; playerSuteHaisCount++) {
    ...
    private void thinkSutehai(Hai addHai) {
        ...
        int maxScore = getCountFormatScore(countFormat);
        ...
        for (int i = 0; i < jyunTehaiLength; i++) {
            ...
            int score = getCountFormatScore(countFormat);
            // System.out.println("score:" + score + ",maxScore:" + maxScore +
    ...
    private final static Hai[] haiTable = new Hai[] { new Hai(Hai.ID_WAN_1),
            new Hai(Hai.ID_WAN_2), new Hai(Hai.ID_WAN_3), new Hai(Hai.ID_WAN_4),
            new Hai(Hai.ID_WAN_5), new Hai(Hai.ID_WAN_6), new Hai(Hai.ID_WAN_7),
            new Hai(Hai.ID_WAN_8), new Hai(Hai.ID_WAN_9), new Hai(Hai.ID_PIN_1),
            new Hai(Hai.ID_PIN_2), new Hai(Hai.ID_PIN_3), new Hai(Hai.ID_PIN_4),
            new Hai(Hai.ID_PIN_5), new Hai(Hai.ID_PIN_6), new Hai(Hai.ID_PIN_7),
            new Hai(Hai.ID_PIN_8), new Hai(Hai.ID_PIN_9), new Hai(Hai.ID_SOU_1),
            new Hai(Hai.ID_SOU_2), new Hai(Hai.ID_SOU_3), new Hai(Hai.ID_SOU_4),
            new Hai(Hai.ID_SOU_5), new Hai(Hai.ID_SOU_6), new Hai(Hai.ID_SOU_7),
            new Hai(Hai.ID_SOU_8), new Hai(Hai.ID_SOU_9), new Hai(Hai.ID_TON),
            new Hai(Hai.ID_NAN), new Hai(Hai.ID_SHA), new Hai(Hai.ID_PE),
            new Hai(Hai.ID_HAKU), new Hai(Hai.ID_HATSU), new Hai(Hai.ID_CHUN) };
    ...
    private int getCountFormatScore(CountFormat countFormat) {
        ...
        for (int i = 0; i < countFormat.m_countNum; i++) {
            ...
            if ((countFormat.m_counts[i].m_noKind & Hai.KIND_SHUU) > 0) {
                if ((countFormat.m_counts[i].m_noKind
                        + 1) == countFormat.m_counts[i + 1].m_noKind) {
                    score += 4;
                }

                if ((countFormat.m_counts[i].m_noKind
                        + 2) == countFormat.m_counts[i + 2].m_noKind) {
                    score += 4;
                }
            }
        }
